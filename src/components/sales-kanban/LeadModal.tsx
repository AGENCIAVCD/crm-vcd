  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] overflow-y-auto border-[#ffb800]/40 bg-[#0b0b0b] text-white">
        <DialogHeader>
          <div className="inline-flex w-fit items-center gap-2 rounded-full bg-[#ffb800] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.18em] text-black">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          <div className="grid gap-4 md:grid-cols-2">
            <label className="space-y-2">
              <span className="text-[10px] font-black uppercase tracking-[0.16em] text-[#ffb800]">
                Nome do cliente *
              </span>
              <input
                value={draft.clientName}
                onChange={(event) => updateDraft("clientName", event.target.value)}
                className="rounded-[14px] border border-white/10 bg-white px-4 py-3 text-sm text-black outline-none transition focus:border-[#ffb800] focus:ring-4 focus:ring-[#ffb800]/20"
          ...
          <label className="block space-y-2">
            ...
            <textarea
              ...
              className="min-h-[160px] rounded-[14px] border border-white/10 bg-white px-4 py-3 text-sm leading-6 text-black outline-none transition focus:border-[#ffb800] focus:ring-4 focus:ring-[#ffb800]/20"
              placeholder="Resumo da conversa, objeções, urgência, próximos passos..."
            />
          </label>

          {feedback ? (
            <div className="rounded-[16px] border border-[#ffb800] bg-[#ffb800]/10 px-4 py-3 text-sm font-semibold text-[#ffb800]">
              {feedback}
            </div>
          ) : null}
          ...
          <DialogFooter>
            <button
              ...
              className="rounded-[8px] border-2 border-white px-4 py-3 text-sm font-black uppercase tracking-[0.08em] text-white transition hover:bg-white hover:text-black"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={isSaving}
              className="inline-flex items-center justify-center gap-2 rounded-[8px] bg-[#ffb800] px-4 py-3 text-sm font-black uppercase tracking-[0.08em] text-black transition hover:bg-[#e5a400] disabled:cursor-not-allowed disabled:opacity-70"
            >
